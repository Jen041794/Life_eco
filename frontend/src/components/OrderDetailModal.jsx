    ...
    <Modal show={show} onHide={onHide} centered size="lg">
      ...
      <Modal.Body>
        <div className="mb-3 d-flex flex-wrap gap-4">
          <div><span className="text-muted">客人：</span>{order.user}</div>
          <div>
            <span className="text-muted">狀態：</span>
            <Badge bg={STATUS_VARIANTS[order.status]}>{STATUS_LABELS[order.status]}</Badge>
          </div>
          <div><span className="text-muted">建立時間：</span>{new Date(order.created_at).toLocaleString()}</div>
        </div>
        ...
        <div className="text-end fs-5">
          總計：<span className="fw-bold text-success">${order.total_price}</span>
        </div>
      </Modal.Body>
    </Modal>
  )
