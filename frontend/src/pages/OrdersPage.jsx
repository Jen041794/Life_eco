...
export default function OrdersPage() {
  ...
  const { data, isLoading, isError, isFetching } = useGetOrdersQuery({ page, status, search })
  ...
  return (
    <>
      ...
      <div className="d-flex flex-wrap gap-3 mb-3">
        ...
        <Form onSubmit={submitSearch} style={{ maxWidth: 320 }} className="flex-grow-1">
          <InputGroup>
            <Form.Control
              placeholder="搜尋客人帳號"
              value={searchInput}
      ...
      <Table hover responsive className="align-middle bg-white shadow-sm">
        ...
        <tbody>
          {orders.length === 0 ? (
            <tr><td colSpan={6} className="text-center text-muted py-4">沒有訂單</td></tr>
          ) : (
            orders.map((o) => {
              ...
              return (
                <tr key={o.id}>
                  ...
                  <td className="small text-muted">{new Date(o.created_at).toLocaleString()}</td>
                  <td>
                    <Button size="sm" variant="outline-secondary" className="me-2" onClick={() => setDetail(o)}>
                      明細
                    </Button>
                    {action && (
                      <Button size="sm" variant="outline-primary" className="me-2"
                        disabled={busy} onClick={() => advance(o)}>
                        {action.label}
                      </Button>
                    )}
                    {CANCELLABLE.includes(o.status) && (
                      <Button size="sm" variant="outline-danger"
                        disabled={busy} onClick={() => cancel(o)}>
                        取消
                      </Button>
                    )}
                  </td>
      ...
      <OrderDetailModal show={Boolean(detail)} onHide={() => setDetail(null)} order={detail} />
    </>
  )
}
