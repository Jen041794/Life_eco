...
function validateForm(form) {
  return {
    ...
    shipping_address: validateAddress(form.shipping_address, {
      required: true,
    }),
  };
}
...
export default function CheckoutPage() {
  ...
  const onPhoneChange = (e) =>
    setField("recipient_phone", onlyDigits(e.target.value).slice(0, 10));
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // 帶訂單編號與明細到完成頁（後端建立回傳不含 items，明細用購物車內容）
      const order = {
        order_number: created.order_number,
        total_price: created.total_price ?? total,
        items: items.map((i) => ({
          id: i.id,
          name: i.name,
          price: i.price,
    ...
    return (
      <div>
        ...
        <Alert variant="secondary">
          購物車是空的，沒有東西可以結帳。<Link to="/shop">去逛逛商品 →</Link>
        </Alert>
      </div>
    );
  }
              ...
              <Form onSubmit={handleSubmit} noValidate>
                ...
                <Form.Group className="mb-4">
                  ...
                  <Form.Control
                    ...
                    onChange={(e) =>
                      setField("shipping_address", e.target.value)
                    }
