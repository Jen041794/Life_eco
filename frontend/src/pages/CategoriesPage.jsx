...
export default function CategoriesPage() {
  ...
  return (
    <>
      ...
      <Table hover responsive className="align-middle bg-white shadow-sm" style={{ maxWidth: 640 }}>
        <thead className="table-light">
          <tr>
            <th>#</th><th>名稱</th><th className="text-center">商品數</th><th style={{ width: 180 }}>操作</th>
          </tr>
        </thead>
        <tbody>
          {categories.length === 0 ? (
            <tr><td colSpan={4} className="text-center text-muted py-4">還沒有分類</td></tr>
          ) : (
            categories.map((c) => (
              <tr key={c.id}>
                <td>{c.id}</td>
                ...
                <td className="text-center">
                  <Badge bg={c.product_count > 0 ? 'info' : 'light'}
                    text={c.product_count > 0 ? undefined : 'dark'} className="fw-normal">
                    {c.product_count}
                  </Badge>
                </td>
                <td>
                  {editingId === c.id ? (
                    <>
                      <Button size="sm" variant="success" className="me-2" onClick={() => saveEdit(c)}>儲存</Button>
                      <Button size="sm" variant="outline-secondary" onClick={cancelEdit}>取消</Button>
                    </>
                  ) : (
                    <>
                      <Button size="sm" variant="outline-primary" className="me-2" onClick={() => startEdit(c)}>改名</Button>
                      <Button size="sm" variant="outline-danger" onClick={() => handleDelete(c)}>刪除</Button>
                    </>
                  )}
                </td>
              </tr>
