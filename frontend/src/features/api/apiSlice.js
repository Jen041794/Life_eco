...
export const apiSlice = createApi({
  ...
  endpoints: (builder) => ({
    ...
    // 付款：body { order: 訂單id, provider: 'mock' }
    // 後端 mock 一律成功 → 建 success 付款紀錄、訂單翻成「已付款」
    // 付完讓 Order（訂單狀態變了）與 Stats（營收）失效，自動刷新
    // 之後接 Stripe 時前端只要換掉 PaymentModal 的表單，這個 endpoint 不用動
    createPayment: builder.mutation({
      query: (body) => ({ url: "/payments/", method: "POST", body }),
      invalidatesTags: ["Order", "Stats"],
    }),

    // Stripe：付款前先跟後端要一張 PaymentIntent，拿回 client_secret 給 Stripe.js 確認
